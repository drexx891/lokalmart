...
export default function CartClient({ initialOrders, initialDiscount }: CartClientProps) {
    const [orders, setOrders] = useState<OrderGroup[]>(initialOrders);
    ...
    const handleUpdateQuantity = async (itemId: string, newQuantity: number) => {
        ...
        setOrders(orders.map(order => ({
            ...order,
            items: order.items.map(i => i.id === itemId ? { ...i, quantity: newQuantity } : i)
        })));
        ...
        if (!result.success) {
            ...
            setOrders(oldOrders); // Rollback if failed
        }
    };
    ...
    const handleRemoveItem = async (itemId: string) => {
        ...
        })).filter(order => order.items.length > 0)); // Hilangkan order jika items kosong
    ...
    if (orders.length === 0 || totalQuantity === 0) {
        return (
            <div className="bg-white border border-[#E5E7EB] rounded-sm py-20 flex flex-col items-center justify-center text-[#9CA3AF] shadow-sm">
                ...
                <p>Temukan produk dari berbagai toko dan tambahkan ke keranjang Anda.</p>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col gap-4">
                ...
                {orders.map((order) => {
                    ...
                    return (
                        <div key={order.id} className="bg-white border border-[#E5E7EB] rounded-sm shadow-sm mb-4 overflow-hidden">
                            {/* Supplier Header */}
                            <div className="bg-[#F9FAFB] border-b border-[#E5E7EB] px-4 py-3 flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#6B7280]">
                                    <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
                                    <polyline points="9 22 9 12 15 12 15 22"></polyline>
                                </svg>
                                <span className="font-bold text-[#1F2937]">
                                    {order.supplier?.companyName || "LokalMart Official"}
                                </span>
                            </div>
                            ...
                            <div className="flex flex-col">
                                {order.items.map((item, index) => {
                                    ...
                                    return (
                                        <div key={item.id} className={`p-4 flex flex-col md:flex-row gap-4 items-center transition-opacity ${!isLast ? 'border-b border-[#E5E7EB]' : ''} ${isLoading ? 'opacity-70' : 'opacity-100'}`}>
                                            <div className="flex-[3] flex items-center gap-4 w-full">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    {item.variant && (
                                                        <div className="text-[11px] text-[#4B5563] bg-[#F3F4F6] px-2 py-1 rounded inline-block mt-1 border border-[#E5E7EB]">
                                                            Varian: <span className="font-semibold">{item.variant.name}</span>
                                                        </div>
                                                    )}
            ...
            <div className="w-full lg:w-[320px] flex-shrink-0">
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-sm p-5 sticky top-6 shadow-sm mt-4 transition-all">
                    <h2 className="text-lg font-bold text-[#1F2937] mb-4 border-b border-[#E5E7EB] pb-3">Ringkasan Pembayaran</h2>
                    
                    <div className="flex justify-between items-center mb-4 text-[#6B7280] text-sm">
                        <span>Total Pesanan dari {orders.filter(o => o.items.length > 0).length} Toko</span>
                    ...
                    <p className="text-[10px] text-[#9CA3AF] text-center leading-relaxed mt-4">
                        Ongkos kirim akan dihitung secara kolektif saat *Checkout* dan disesuaikan berdasarkan lokasi masing-masing toko.
                    </p>
                </div>
            </div>
        </div>
